...
export default function SphereSnakeGame() {
  ...
  const [collisionType, setCollisionType] = useState<"barrier" | "self">(
    "barrier"
  );
  ...
  useEffect(() => {
    ...
    const initGame = () => {
      ...
      headingTangent
        .sub(headNormal.clone().multiplyScalar(headingTangent.dot(headNormal)))
        .normalize();
      ...
      for (let i = 0; i < initialSegments * 3; i++) {
        ...
        const axis = new THREE.Vector3()
          .crossVectors(headNormal, backwardTangent)
          .normalize();
      ...
      // Create dots
      const { dots, respawnDot } = createDots(
        scene,
        dotCount,
        dotRadius,
        dotSurfaceRadius,
      ...
      const tick = (t: number) => {
        ...
        if (steer !== 0) {
          headingTangent = rotateAroundAxis(
            headingTangent,
            headNormal,
            steer * steerSpeed * dt
          ).normalize();
          ...
          headingTangent
            .sub(
              headNormal.clone().multiplyScalar(headingTangent.dot(headNormal))
            )
            .normalize();
        }
        ...
        headMesh.lookAt(
          tmpVec.copy(headNormal).multiplyScalar(snakeRadius + 1)
        );
        ...
        for (let i = 0; i < pointsToUse; i++) {
          tubePathPoints.push(
            segmentNormals[i].clone().multiplyScalar(snakeRadius)
          );
        }
        ...
        if (segmentCount > initialSegments + 5) {
          ...
          for (let i = startCheck; i < endCheck; i++) {
            const segmentPos = segmentNormals[i]
              .clone()
              .multiplyScalar(snakeRadius);
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-linear-to-b from-slate-900 to-black">
          <div className="text-center">
            ...
            <p className="text-white/60 text-lg mb-12">
              Navigate the sphere, avoid the barriers
            </p>
            ...
            <div className="bg-slate-800/50 rounded-xl p-8 backdrop-blur-sm mb-8">
              <div className="mb-8 pb-8 border-b border-white/10">
                ...
                <p className="text-white/50 text-sm mt-4">
                  {difficulty === "easy" &&
                    "Slower speed (75%), fewer barriers"}
                  {difficulty === "medium" && "Normal speed, standard barriers"}
                  {difficulty === "hard" &&
                    "Faster speed (125%), more barriers"}
                </p>
              </div>
              <div className="mb-6">
                <h3 className="text-white text-xl font-semibold mb-4">
          ...
          <div className="text-center max-w-2xl">
            ...
            <div className="bg-slate-800/50 rounded-xl p-8 backdrop-blur-sm mb-8">
              ...
              <p className="text-white/40 text-sm mt-4">
                Unlock new skins, power-ups, and more!
              </p>
            </div>
            <button
              onClick={() => setMenuScreen("main")}
        ...
        <>
          ...
          <div className="pointer-events-none absolute left-4 top-4 rounded-lg bg-black/40 px-3 py-2 text-xs text-white/90 backdrop-blur">
            ...
            <span className="font-semibold">
              {controlType === "keyboard"
                ? "↑ forward | ← / → steer"
                : "L-Click forward | Move Mouse to steer"}
            </span>{" "}
            | <span className="font-semibold">P</span> to pause
          </div>
        </>
      )}
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="rounded-xl bg-red-900/80 px-12 py-8 text-center backdrop-blur-sm border-2 border-red-500">
            ...
            <div className="text-xl text-white/80 mb-6">
              Final Score:{" "}
              <span className="font-bold tabular-nums">{dotsEaten}</span>
            </div>
            <div className="flex gap-4 justify-center mt-6">
              <button
